    ...
    <div class="page-container">
      ...
      <van-swipe class="my-swipe" :autoplay="3000" indicator-color="white">
        <van-swipe-item
          ...
          :key="item.url"
        >
          <van-image fit="cover" :src="item.url" width="100vw" height="35vh">
...
const getRoomDetailHandle = async () => {
  ...
  // 转换图片URL：将完整的MinIO地址转换为代理路径
  if (data.graphVoList) {
    data.graphVoList = data.graphVoList.map((item: any) => {
      // 如果是完整的http地址，转换为代理路径
      if (item.url && item.url.startsWith('http')) {
        // 提取 /bucketName/filename 部分
        const urlObj = new URL(item.url);
        const path = urlObj.pathname; // 例如: /lease/20260522/xxx.jpg
        item.url = '/minio' + path;
      }
      return item;
    });
