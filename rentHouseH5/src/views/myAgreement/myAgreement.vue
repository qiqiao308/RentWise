...
const getAgreementListHandle = async () => {
  ...
  if (data) {
    data.forEach((item: any) => {
      if (item.roomGraphVoList) {
        item.roomGraphVoList = item.roomGraphVoList.map((graphItem: any) => {
          if (graphItem.url && graphItem.url.startsWith('http')) {
            const urlObj = new URL(graphItem.url);
            const path = urlObj.pathname;
            return { ...graphItem, url: '/minio' + path };
          }
          return graphItem;
        });
      }
